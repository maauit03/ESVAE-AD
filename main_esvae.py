...
def calc_autoencoder_frechet_distance(network, epoch):
    ...
    if glv.network_config['dataset'] == "MNIST":
        dataset = 'mnist'
    elif glv.network_config['dataset'] == "MVTEC":
        dataset = 'MVTEC'
...
if __name__ == '__main__':
    ...
    default_save_path = os.path.join(os.path.dirname(__file__), 'data/ESVAE/')
    parser.add_argument('-project_save_path', type=str, default=default_save_path,
    ...
    if network_config['model'] == 'ESVAE':
        ...
        net = esvae.ESVAELarge(device=init_device, distance_lambda=distance_lambda, mmd_type=mmd_type, input_size=64)
